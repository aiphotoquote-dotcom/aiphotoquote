...
async function postJson<T>(url: string, body: any, opts?: { signal?: AbortSignal }): Promise<T> {
...
export default function QuoteForm({
  ...
  // Tenant allows rendering -> default opt-in ON (customer can uncheck)
  ...
  }, [tenantSlug, aiRenderingEnabled, aiRenderOptIn, images.length, result.quoteLogId, renderState.status, isUploading, isSubmitting]);
  ...
  // ---------- submit estimate (with HARD TIMEOUT) ----------
  async function submitEstimate() {
    ...
    // Scroll up so the user sees the progress bars
    document.getElementById("apq-progress")?.scrollIntoView({ behavior: "smooth", block: "start" });
    ...
    try {
      ...
      const j = await postJson<any>("/api/quote/submit", payload, { signal: controller.signal });
      ...
      if (e?.name === "AbortError") {
        throw new Error("Estimate is taking too long. Please try again (server timeout).");
      }
      throw e;
    } finally {
  ...
  return (
    <div className="w-full">
      ...
      <div id="apq-progress" className="space-y-3">
        <ProgressBar
          labelLeft="Progress"
          labelRight={
            isUploading ? "Uploading…" : isSubmitting ? "Working…" : estimateReady ? "Estimate ready" : "Add photos"
          }
          active={isUploading || isSubmitting}
        />
        ...
        <div className="mt-4 flex flex-wrap gap-3">
          <button
            ...
            className={cn(
              ...
              (isSubmitting || isUploading) ? "opacity-60" : ""
            )}
