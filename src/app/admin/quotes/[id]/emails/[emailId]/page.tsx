...
async function findQuoteEmailById(args: {
  tenantId: string;
  quoteId: string;
  emailId: string;
...
/**
 * If the URL "emailId" actually belongs to quote_email_sends,
 * resolve it to the real stored email (quote_emails) via provider_message_id.
 *
 * This avoids schema changes and makes click-to-view work immediately.
 */
async function resolveViaSendRow(args: {
  tenantId: string;
  quoteId: string;
  sendId: string;
}): Promise<EmailRow | null> {
  ...
  const r1 = await db.execute(sql`
    ...
    where id = ${args.sendId}::uuid
      and tenant_id = ${args.tenantId}::uuid
      and quote_log_id = ${args.quoteId}::uuid
    limit 1
  `);

  const sendRow: any = (r1 as any)?.rows?.[0] ?? (Array.isArray(r1) ? (r1 as any)[0] : null);
  const msgId = safeTrim(sendRow?.provider_message_id);
  if (!msgId) return null;

  // 2) find the stored email by provider_message_id
  ...
  const emailRow: any = (r2 as any)?.rows?.[0] ?? (Array.isArray(r2) ? (r2 as any)[0] : null);
  return emailRow ? (emailRow as EmailRow) : null;
}
...
export default async function QuoteEmailViewPage(props: {
  ...
  // ✅ Attempt 1: treat emailId as quote_emails.id
  let row = await findQuoteEmailById({ tenantId, quoteId, emailId });

  // If found but has no bodies, treat as "not good enough" and try resolving via send row
  const htmlMaybe = safeTrim(row?.html ?? "");
  const textMaybe = safeTrim(row?.text ?? "");
  const hasBodies = Boolean(htmlMaybe || textMaybe);

  // ✅ Attempt 2: treat emailId as quote_email_sends.id and resolve via provider_message_id
  if (!row || !hasBodies) {
    const resolved = await resolveViaSendRow({ tenantId, quoteId, sendId: emailId });
...
{JSON.stringify(safeJson(row.meta) ?? row.meta ?? {}, null, 2)}
      ...
      <div className="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-gray-950/40">
        ...
        <div className="mt-2 text-xs text-gray-600 dark:text-gray-400">This is read-only. Scripts are blocked.</div>
