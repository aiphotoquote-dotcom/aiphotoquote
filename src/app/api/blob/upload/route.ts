...
export default function QuoteForm({
  ...
  async function uploadFiles(files: FileList) {
    ...
    // sequential to keep memory stable on mobile
    for (const file of Array.from(files)) {
      const res = await upload(file.name, file, {
        access: "public",
        handleUploadUrl: "/api/blob/upload",
      });
      if (res?.url) outUrls.push(String(res.url));
    }

    if (!outUrls.length) throw new Error("Upload returned no file urls");
    addImagesFromUrls(outUrls);
  }
