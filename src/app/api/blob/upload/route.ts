...
function safeExtFromType(mime: string | undefined) {
...
export default function QuoteForm({
  ...
  // ---------- upload (DIRECT to Vercel Blob, no 413) ----------
  async function uploadFilesDirect(files: FileList) {
    if (!files?.length) return;

    const urls: string[] = [];

    for (const file of Array.from(files)) {
      // still keep a sane cap to avoid a user selecting 100 photos
      if (images.length + urls.length >= 12) break;

      const ext = safeExtFromType(file.type);
      const filename = `${Date.now()}-${Math.random().toString(16).slice(2)}.${ext}`;

      // Browser uploads directly to Vercel Blob via signed upload route
      const blob = await upload(`quotes/${filename}`, file, {
        access: "public",
        ...
        <div className="mt-3 flex flex-col gap-2">
          <label className="inline-flex w-fit cursor-pointer items-center justify-center rounded-md bg-neutral-900 px-4 py-2 text-sm font-medium text-white">
            ...
            <input
              ...
              onChange={async (e) => {
                try {
                  if (e.target.files) await uploadFilesDirect(e.target.files);
                } catch (err: any) {
