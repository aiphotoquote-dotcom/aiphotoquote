// src/app/api/onboarding/industry-defaults-by-tenant/route.ts
...
function makeGeneric(industryKey: string): Defaults {
  return {
    industryKey,
    subIndustries: [],
    commonServices: ["Consultation / estimate", "Repair / replacement", "New install / build", "Labor + materials"],
    commonPhotoRequests: [
      "Wide shot of the full job area",
      "Close-ups of damage / detail areas",
...
const PRESETS: Record<string, Defaults> = {
  ...
  paving_contractor: {
    ...
    commonServices: ["New asphalt / overlay", "Sealcoating", "Crack fill / patching", "Re-striping (commercial)", "Grading & base prep"],
    commonPhotoRequests: [
  ...
  automotive_repair: {
    ...
    commonServices: ["Damage assessment", "Panel repair/replacement", "Paint & refinish", "Mechanical repair (varies by shop)", "Insurance estimate support (if offered)"],
    commonPhotoRequests: [
...
export async function GET(req: Request) {
  try {
    ...
    // Read current selected industry from tenant_settings
    const r = await db.execute(sql`
      select industry_key
    ...
    const row = firstRow(r);
    const industryKey = normalizeIndustryKey(row?.industry_key ?? "");

    if (!industryKey) {
      return NextResponse.json(
        { ok: false, error: "NO_INDUSTRY", message: "No industry selected yet for this tenant." },
        { status: 400 }
      );
    }

    const defaults = PRESETS[industryKey] ?? makeGeneric(industryKey);

    return NextResponse.json({ ok: true, tenantId, industryKey, defaults }, { status: 200 });
  } catch (e: any) {
