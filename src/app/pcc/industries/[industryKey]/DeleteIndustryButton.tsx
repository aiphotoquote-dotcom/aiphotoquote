...
export default function DeleteIndustryButton(props: { industryKey: string }) {
  ...
  async function run() {
    ...
    const confirmText = safeTrim(prompt(`Type DELETE to permanently delete "${industryKey}" (hard delete).`) ?? "");
    ...
    try {
      ...
      // If it deleted, this page likely 404s now; go back to list.
  ...
  return (
    <div className="space-y-2">
      <button
        ...
        title="Hard delete this industry (blocked if any tenants still assigned). Audit logged."
      >
