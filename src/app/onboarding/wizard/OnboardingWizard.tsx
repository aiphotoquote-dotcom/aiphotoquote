...
export default function OnboardingWizard() {
  ...
  async function saveStep1(payload: {
    businessName: string;
    ownerName: string;
    ownerEmail: string;
    website?: string;
...
function Step1({
  ...
  onSubmit: (payload: { businessName: string; ownerName: string; ownerEmail: string; website?: string }) => Promise<void>;
}) {
  ...
  const can = businessName.trim().length >= 2 && ownerName.trim().length >= 2 && ownerEmail.trim().includes("@");
